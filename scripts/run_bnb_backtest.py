#!/usr/bin/env python3
"""Run backtesting and paper trading simulation for the EMAConfluenceETH strategy.

Usage:
    python scripts/run_bnb_backtest.py [--timeframe 1h|15m] [--equity 10000] [--risk 0.01]
"""
...
def calculate_extended_metrics(report, data, timeframe):
    ...
    if len(data) > 0:
        ...
        if len(trade_returns) > 1:
            ...
            # Annualize assuming trade distribution matches time in market
...
def main():
    parser = argparse.ArgumentParser(description="Run BNBStrategy backtest.")
    parser.add_argument("--timeframe", type=str, choices=["1h", "15m"], default="1h", help="Timeframe of the data")
    ...
    data_path = os.path.join(project_root, 'data', f'BNBUSDT_{args.timeframe}.csv')
    ...
    try:
        ...
        if 'timestamp' in data.columns:
            ...
            # Try to guess index or convert first column
    ...
    backtester = WalkForwardBacktester(
        strategy_factory=BNBStrategy,
        starting_equity=args.equity,
        risk_per_trade_pct=args.risk,
    )

    report = backtester.run(data, symbol="BNBUSDT", warmup_bars=250)

    metrics = calculate_extended_metrics(report, data, args.timeframe)

    # Paper trading simulation: generate signal on the latest data
    strategy = BNBStrategy(data)
    last_signal = strategy.generate_signals(symbol="BNBUSDT")
    if last_signal is not None:
        last_signal["timestamp"] = data.index[-1].isoformat()

    results = {
        "strategy": "BNBStrategy",
    ...
    results_path = os.path.join(results_dir, "bnb_backtest.json")
