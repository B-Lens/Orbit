#!/usr/bin/env python3
"""Backtest BNBUSDT hourly candles with conservative execution assumptions."""
...
from orbit.strategies.bnbusdt_strategy import BNBUSDTStrategy  # noqa: E402
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    # Monkeypatch to avoid latest_closed check during walk forward
    def factory(frame):
        s = BNBUSDTStrategy(frame)
        s._hourly_data = lambda: (s.data, True) 
        return s
    ...
    report = WalkForwardBacktester(
        ...
        fee_rate=0.0004,
        slippage_bps=2.0,
    ).run(data, symbol="BNBUSDT", warmup_bars=250)
    
    result = {
        "strategy": "BNBUSDTStrategy",
        "symbol": "BNBUSDT",
        "timeframe": "1h",
