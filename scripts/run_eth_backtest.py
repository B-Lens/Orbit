...
Usage:
    python scripts/run_eth_backtest.py [--timeframe 1h|15m] [--equity 10000] [--risk 0.01]
"""
...
def main():
    parser = argparse.ArgumentParser(description="Run EMAConfluenceETH backtest.")
    ...
    data_path = os.path.join(project_root, 'data', f'ETHUSDT_{args.timeframe}.csv')
    ...
    report = backtester.run(data, symbol="ETHUSDT", warmup_bars=250)

    metrics = calculate_extended_metrics(report, data, args.timeframe)

    # Paper trading simulation: generate signal on the latest data
    strategy = EMAConfluenceETH(data)
    last_signal = strategy.generate_signals(symbol="ETHUSDT")
    ...
    results_path = os.path.join(results_dir, "eth_ema_confluence_backtest.json")
