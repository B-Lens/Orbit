...
class _RetryableOpenAIError(RuntimeError):
    """A transient OpenAI transport failure that is safe to retry."""
...
class CodexOAuthResponsesClient:
    ...
    def _invoke_stream(self, prompt: str, web_search: bool) -> Optional[str]:
        ...
        try:
            ...
            message = f"OpenAI HTTP {error.code}.{hint}"
            ...
            raise _RetryableOpenAIError("OpenAI request transport failed") from error
