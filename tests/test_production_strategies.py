...
def _link_fresh_hour(data: pd.DataFrame) -> pd.Timestamp:
...
class TestLINKUSDTStrategy(unittest.TestCase):
    ...
    def test_insufficient_bars_returns_none(self, _mock_discord):
        data = _linkusdt_hourly_data(bars=50)
    ...
    def test_flat_market_returns_none(self, _mock_discord):
        data = _linkusdt_hourly_data(direction="flat")
        with patch.object(
            LINKUSDTStrategy, "_current_hour", return_value=_link_fresh_hour(data)
        ):
    ...
    def test_buy_signal_has_correct_reward_risk(self, _mock_discord):
        data = _linkusdt_hourly_data(direction="up")
        with patch.object(LINKUSDTStrategy, "_current_hour", return_value=_link_fresh_hour(data)):
    ...
    def test_sell_signal_has_correct_reward_risk(self, _mock_discord):
        data = _linkusdt_hourly_data(direction="down")
        with patch.object(LINKUSDTStrategy, "_current_hour", return_value=_link_fresh_hour(data)):
            strategy = LINKUSDTStrategy(data)
            signal = strategy.generate_signals(symbol="LINKUSDT")
