...
type CommandCenter = { generated_at: string; runtime: RuntimeState; positions: Position[]; signals: Signal[]; sentiment: Sentiment; risk_execution: RiskExecution; logs: LogEntry[]; exceptions: ExceptionEntry[] };
    ...
    <div className="workspace" id="top">
      ...
      <main className="command-center">
        ...
        <section className="primary-grid">
          ...
          <article className="panel sentiment-panel" id="sentiment"><header className="panel-header"><div><BrainCircuit size={17} /><span><h2>Market sentiment</h2><p>Latest completed intelligence run</p></span></div><small>{age(snapshot?.sentiment.updated_at ?? null)} ago</small></header>{snapshot?.sentiment.effective == null ? <Empty>No sentiment result has been published.</Empty> : <div className="sentiment-content"><div className={`sentiment-score ${tone(snapshot.sentiment.effective)}`}><strong>{sentimentScore == null ? "—" : `${sentimentScore}%`}</strong><span>{snapshot.sentiment.effective}</span></div><dl><div><dt>Observed</dt><dd>{snapshot.sentiment.observed ?? "—"}</dd></div><div><dt>Provider</dt><dd>{snapshot.sentiment.provider ?? "—"}</dd></div><div><dt>Action</dt><dd>{snapshot.sentiment.action ?? "—"}</dd></div></dl><p>{snapshot.sentiment.explanation ?? "No explanation was provided by the sentiment workflow."}</p></div>}</article>
