...
import { startOfIstDay as startOfDay, istDateValue as dateInputValue, istMidnight, moveIstDay, formatIst } from "./reportCalendar";
...
type ClosedTrade = { trade_id: string; symbol: string; side: string | null; quantity: number | null; entry_price: number | null; exit_price: number | null; pnl: number | null; execution_mode: string | null; entered_at: string | null; closed_at: string | null; duration_seconds: number | null; pnl_source: string | null; lifecycle_scope: string | null; llm_exit_reasoning: Record<string, unknown> | null; details: Record<string, unknown> };
...
type CommandCenter = { generated_at: string; runtime: RuntimeState; positions: Position[]; signals: Signal[]; closed_trades: ClosedTrade[]; sentiment: Sentiment; sentiment_history: SentimentHistory[]; risk_execution: RiskExecution; logs: LogEntry[]; exceptions: ExceptionEntry[]; older_exception_count: number };
...
const ageWithSuffix = (value: string | null) => value ? `${age(value)} ago` : "Not reported";
...
const dayLabel = (value: Date) => { const today = startOfDay(new Date()); const daysAgo = Math.round((today.getTime() - value.getTime()) / 86_400_000); if (daysAgo === 0) return "Today"; if (daysAgo === 1) return "Yesterday"; return value.toLocaleDateString(undefined, { timeZone: "Asia/Kolkata", weekday: "short", day: "numeric", month: "short", year: value.getFullYear() === today.getFullYear() ? undefined : "numeric" }); };
...
function CommandCenterPage() {
  ...
  const [closedTradeDay, setClosedTradeDay] = useState(() => startOfDay(new Date()));
  const [followingToday, setFollowingToday] = useState(true);
  const [loadedClosedTradeDay, setLoadedClosedTradeDay] = useState<string | null>(null);
  const [closedTradesLoading, setClosedTradesLoading] = useState(true);
  const requestGeneration = useRef(0);

  const load = useCallback(async (showClosedTradesLoading = false) => { const generation = ++requestGeneration.current; const requestedDay = dateInputValue(closedTradeDay); if (showClosedTradesLoading) setClosedTradesLoading(true); try { const end = moveIstDay(closedTradeDay, 1); const params = new URLSearchParams({ signal_limit: "100", closed_trade_limit: "250", closed_trade_start: closedTradeDay.toISOString(), closed_trade_end: end.toISOString(), log_limit: "100", exception_limit: "25" }); const response = await fetch(`/api/command-center?${params}`); if (!response.ok) throw new Error(response.status === 503 ? "Operational data store is unavailable." : `Command center request failed (${response.status}).`); const nextSnapshot = await response.json() as CommandCenter; if (generation !== requestGeneration.current) return; setSnapshot(nextSnapshot); setLoadedClosedTradeDay(requestedDay); setError(""); } catch (requestError) { if (generation !== requestGeneration.current) return; setError(requestError instanceof Error ? requestError.message : "Live state is unavailable."); } finally { if (generation === requestGeneration.current) { setLoading(false); setClosedTradesLoading(false); } } }, [closedTradeDay]);
  useEffect(() => { document.documentElement.dataset.theme = theme; localStorage.setItem("orbit-theme", theme); }, [theme]);
  useEffect(() => { void load(true); if (!autoRefresh) return; const timer = window.setInterval(() => { const today = startOfDay(new Date()); if (followingToday && closedTradeDay.getTime() !== today.getTime()) { setClosedTradeDay(today); return; } void load(); }, REFRESH_INTERVAL); return () => window.clearInterval(timer); }, [load, autoRefresh, closedTradeDay, followingToday]);
  ...
  const configuredProtectionCount = snapshot?.positions.filter((item) => item.stop_loss != null).length ?? 0;
  ...
  const isToday = closedTradeDay.getTime() === startOfDay(new Date()).getTime();
  const selectClosedTradeDay = (day: Date) => { const next = startOfDay(day); setFollowingToday(next.getTime() === startOfDay(new Date()).getTime()); setClosedTradeDay(next); };
  const moveClosedTradeDay = (offset: number) => { const next = moveIstDay(closedTradeDay, offset); selectClosedTradeDay(next); };
  const closedTrades = loadedClosedTradeDay === dateInputValue(closedTradeDay) ? snapshot?.closed_trades ?? [] : [];

  const closedNetPnl = closedTrades.some((item) => item.pnl == null) ? null : closedTrades.reduce((sum, item) => sum + (item.pnl ?? 0), 0);
  ...
  return <div className="app-shell">
    <aside className="sidebar">
      ...
      <nav aria-label="Command center sections">
        ...
        <a className="nav-item" href="#closed-trades"><CheckCircle2 size={17} />Closed trades <b>{closedTrades.length}</b></a>
    ...
    <div className="workspace" id="top">
      ...
      <main className="command-center">
        ...
        <section className="now-strip"><span className={`now-orb ${live ? "online" : ""}`}><Activity size={20} /></span><div className="now-copy"><small>ORBIT IS NOW</small><strong>{snapshot?.runtime.current_activity ?? (loading ? "Connecting to the trading runtime…" : "Waiting for runtime activity")}</strong><p>{snapshot?.runtime.detail ?? "The runtime has not published a current activity detail."}</p></div><div className="now-meta"><span><small>LAST PROGRESS</small><strong>{ageWithSuffix(snapshot?.runtime.updated_at ?? null)}</strong></span><span><small>RUNTIME</small><strong className={live ? "positive" : "negative"}>{snapshot?.runtime.status ?? "unknown"}</strong></span></div></section>
        ...
        <section className="primary-grid">
          ...
          <article className="panel closed-trades-panel" id="closed-trades"><header className="panel-header closed-trades-header"><div><CheckCircle2 size={17} /><span><h2>Closed trades · {dayLabel(closedTradeDay)}</h2><p>Midnight to midnight IST</p></span></div><div className="day-navigation"><button onClick={() => moveClosedTradeDay(-1)} aria-label="Previous day"><ChevronLeft size={15} /></button><label><CalendarDays size={13} /><input type="date" value={dateInputValue(closedTradeDay)} max={dateInputValue(new Date())} aria-label="Closed trades date" onChange={(event) => { const [year, month, day] = event.target.value.split("-").map(Number); if (year && month && day) selectClosedTradeDay(istMidnight(event.target.value)); }} /></label><button onClick={() => moveClosedTradeDay(1)} disabled={isToday} aria-label="Next day"><ChevronRight size={15} /></button><strong>{closedTradesLoading ? "…" : closedTrades.length} closed</strong></div></header><div className="closed-pnl-summary"><span>Net P&amp;L · shown closed trades{closedTrades.length === 250 ? " (250-row limit)" : ""}</span><strong className={closedNetPnl != null && closedNetPnl < 0 ? "negative" : "positive"}>{closedTradesLoading ? "Loading…" : closedNetPnl == null ? "Unavailable: missing trade P&L" : `${number(closedNetPnl, 8)} USDT`}</strong></div>{closedTradesLoading ? <Empty title="Loading trades">Loading closed trades for {dayLabel(closedTradeDay).toLowerCase()}.</Empty> : !closedTrades.length ? <Empty>No completed trades were closed on {dayLabel(closedTradeDay).toLowerCase()}.</Empty> : <div className="table-scroll permanent-scroll"><table><thead><tr><th>Trade</th><th>Size</th><th>Entry / Exit</th><th>Net P&amp;L</th><th>Duration</th><th>Closed</th><th>Complete record</th></tr></thead><tbody>{closedTrades.map((item) => <tr key={item.trade_id}><td><strong>{item.symbol}</strong><span className={tone(item.side)}>{item.side ?? "—"} · {item.execution_mode ?? "unknown"}</span><span title={item.trade_id}>ID: {item.trade_id}</span></td><td>{number(item.quantity, 6)}</td><td><strong>{price(item.entry_price)}</strong><span>{price(item.exit_price)}</span></td><td className={(item.pnl ?? 0) >= 0 ? "positive" : "negative"}><strong>{item.pnl != null && item.pnl >= 0 ? "+" : ""}{number(item.pnl, 8)} USDT</strong><span>{item.pnl_source ?? "Source not recorded"}</span></td><td>{item.duration_seconds == null ? "—" : `${Math.round(item.duration_seconds / 60)}m`}</td><td><strong>{item.closed_at ? formatIst(item.closed_at) + " IST" : "—"}</strong><span>{age(item.closed_at)} ago</span></td><td><details className="trade-details"><summary>View all fields</summary><pre>{JSON.stringify(item.details, null, 2)}</pre></details></td></tr>)}</tbody></table></div>}</article>
          ...
          <article className="panel exceptions-panel" id="exceptions"><header className="panel-header"><div><FileWarning size={17} /><span><h2>Exceptions <b>{snapshot?.exceptions.length ?? 0}</b></h2><p>Failures from the last 24 hours</p></span></div>{snapshot?.exceptions.length || snapshot?.older_exception_count ? <AlertTriangle className="warning" size={16} /> : <CheckCircle2 className="positive" size={16} />}</header>{!snapshot?.exceptions.length ? <div className={`all-clear ${snapshot?.older_exception_count ? "warning" : ""}`}>{snapshot?.older_exception_count ? <AlertTriangle size={27} /> : <CheckCircle2 size={27} />}<strong>{snapshot?.older_exception_count ? "No recent exceptions" : "No exceptions"}</strong><span>{snapshot?.older_exception_count ? `${snapshot.older_exception_count} older exception${snapshot.older_exception_count === 1 ? " remains" : "s remain"} in the observability buffer.` : "No runtime failures are currently recorded."}</span></div> : <div className="exception-list">{snapshot.exceptions.map((item) => <details key={item.id}><summary><AlertTriangle size={15} /><span><strong>{item.type}</strong><small>{item.message}</small></span><time>{age(item.created_at)} ago</time></summary>{(item.context || item.traceback) && <pre>{item.context}{item.traceback ? `\n${item.traceback}` : ""}</pre>}</details>)}</div>}</article>
        </section>
      </main>
    </div>
  </div>;
