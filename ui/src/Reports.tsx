...
type Report = { report_type: ReportKind; period_start: string; period_end: string; timezone: string; body: string; metrics: { net_pnl: number; equity_value: number | null; opening_equity: number | null; equity_change_pct: number | null; max_drawdown: number; max_drawdown_pct: number | null; realized_pnl: number; commission: number; funding: number; other_income: number } };
...
export function ReportPage({ kind }: { kind: ReportKind }) {
  ...
  return <div className="report-shell">
    ...
    <main className="report-page">
      ...
      <article className="report-document">{loading && !report ? <div className="report-loading"><RefreshCw className="spinning" /><span>Building report from MongoDB ledgers…</span></div> : report ? <><div className="report-period"><FileText size={16} /><span>{formatIst(report.period_start)} to {formatIst(report.period_end)} {report.timezone} (end exclusive)</span></div><div className="markdown-report"><ReactMarkdown remarkPlugins={[remarkGfm]}>{report.body}</ReactMarkdown></div></> : null}</article>
    </main>
  </div>;
}
